...
class panels_renderer_standard
{
  ...
  public function init($plugin, &$display)
  {
      ...
      if (empty($this->plugins['layout'])) {
          watchdog('panels', "Layout: @layout couldn't been found, maybe the theme is disabled.", ['@layout' => $display->layout]);
      }
  }
   ...
   */
  public function prepare_panes($panes)
  {
   ...
   */
  public function prepare_regions($region_pane_list, $settings)
  {
    ...
    $default = [
      ...
      'style settings' => isset($settings['style_settings']['default']) ? $settings['style_settings']['default'] : [],
    ];
      ...
      foreach ($this->plugins['layout']['panels'] as $region_id => $title) {
        ...
        $panes = !empty($region_pane_list[$region_id]) ? $region_pane_list[$region_id] : [];
      ...
      foreach ($this->plugins['layout']['panels'] as $region_id => $title) {
        ...
        $panes = !empty($region_pane_list[$region_id]) ? $region_pane_list[$region_id] : [];
          ...
          if (empty($settings[$region_id]['style']) || $settings[$region_id]['style'] == -1) {
              ...
              $regions[$region_id]['style settings'] = isset($settings['style_settings'][$region_id]) ? $settings['style_settings'][$region_id] : [];
          }
          $regions[$region_id]['pids'] = $panes;
      }
      }
   ...
   * @see drupal_add_css
   */
  public function add_css($filename, $type = 'module', $media = 'all', $preprocess = true)
  {
   ...
   */
  public function render_pane_content(&$pane)
  {
    ...
    if (!is_array($this->display->context)) {
      ...
      if ($caching && ($cache = panels_get_cached_content($this->display, $this->display->args, $this->display->context, $pane))) {
          ...
          $content = ctools_content_render($pane->type, $pane->subtype, $pane->configuration, [], $this->display->args, $this->display->context);
          if (empty($content)) {
              return;
          }
