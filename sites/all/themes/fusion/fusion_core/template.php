    ...
    if ($vars['primary_links']) {
      ...
      if (module_exists('i18nmenu')) {
          ...
          $vars['primary_links_tree'] = menu_tree(variable_get('menu_primary_links_source', 'primary-links'));
            ...
            $vars['primary_links_tree'] = theme('links', $vars['primary_links'], ['class' => 'menu']);
        }
    }
...
function fusion_core_username($object)
{
    if ((!$object->uid) && $object->name) {
        $output = (!empty($object->homepage)) ? l($object->name, $object->homepage, ['attributes' => ['rel' => 'nofollow']]) : check_plain($object->name);
...
function fusion_core_theme()
{
    return [
    'grid_row' => [
      'arguments' => ['element' => null, 'name' => null, 'class' => null, 'width' => null],
    ],
    'grid_block' => [
      'arguments' => ['element' => null, 'name' => null],
    ],
  ];
}
...
function fusion_core_set_regions($grid_width, $sidebar_first_width, $sidebar_last_width)
{
    ...
    $regions = [
    'header_top'        => ['width' => $grid_width, 'total' => count(block_list('header_top')), 'count' => 0],
    'header'            => ['width' => $grid_width, 'total' => count(block_list('header')), 'count' => 0],
    'preface_top'       => ['width' => $grid_width, 'total' => count(block_list('preface_top')), 'count' => 0],
    'preface_bottom'    => ['width' => $grid_width - $sidebar_first_width, 'total' => count(block_list('preface_bottom')), 'count' => 0],
    'sidebar_first'     => ['width' => $sidebar_first_width, 'total' => count(block_list('sidebar_first')), 'count' => 0],
    'content_top'       => ['width' => $grid_width - $sidebar_total, 'total' => count(block_list('content_top')), 'count' => 0],
    'content'           => ['width' => $grid_width - $sidebar_total, 'total' => count(block_list('content')), 'count' => 0],
    'node_top'          => ['width' => $grid_width - $sidebar_total, 'total' => count(block_list('node_top')), 'count' => 0],
    'node_bottom'       => ['width' => $grid_width - $sidebar_total, 'total' => count(block_list('node_bottom')), 'count' => 0],
    'content_bottom'    => ['width' => $grid_width - $sidebar_total, 'total' => count(block_list('content_bottom')), 'count' => 0],
    'sidebar_last'      => ['width' => $sidebar_last_width, 'total' => count(block_list('sidebar_last')), 'count' => 0],
    'postscript_top'    => ['width' => $grid_width - $sidebar_first_width, 'total' => count(block_list('postscript_top')), 'count' => 0],
    'postscript_bottom' => ['width' => $grid_width, 'total' => count(block_list('postscript_bottom')), 'count' => 0],
    'footer'            => ['width' => $grid_width, 'total' => count(block_list('footer')), 'count' => 0],
  ];
...
function fusion_core_edit_links($block)
{
  ...
  if ($block->module == 'block') {
      $text = t('edit block');
      $block_info = ['@region' => str_replace('_', ' ', $block->region)];
      $attributes = ['title' => t('edit the content of this Custom block (in @region)', $block_info), 'class' => 'fusion-block-edit'];
  } else {
      $text = t('configure block');
      $block_info = ['@type' => ucwords($block->module), '@region' => str_replace('_', ' ', $block->region)];
      $attributes = ['title' => t('configure this @type block (in @region)', $block_info), 'class' => 'fusion-block-config'];
  }
    $edit_links[] = l($text, $path, ['attributes' => $attributes, 'query' => $return]);
  ...
  if (user_access('administer menu') && ($block->module == 'menu' || ($block->module == 'user' && $block->delta == 1))) {
      ...
      $attributes = ['title' => t('edit the menu of this @type block (in @region)', $block_info), 'class' => 'fusion-edit-menu'];
      $edit_links[] = l($text, $path, ['attributes' => $attributes, 'query' => $return]);
  }
