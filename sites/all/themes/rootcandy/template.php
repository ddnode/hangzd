...
function _rootcandy_admin_navigation()
{
  ...
  global $user;
    ...
    if (!$rootcandy_navigation) {
        if (!$user->uid) {
            $menu_tree[] = ['href' => 'user/login', 'title' => t('User login')];
        }
    } elseif ($rootcandy_navigation == '_rootcandy_default_navigation') {
        // build default menu
    $menu_tree[] = ['href' => 'admin', 'title' => t('Dashboard')];
        $menu_tree[] = ['href' => 'admin/content', 'title' => t('Content')];
        if (variable_get('node_admin_theme', '0')) {
            $menu_tree[] = ['href' => 'node/add', 'title' => t('Create content')];
        }
        $menu_tree[] = ['href' => 'admin/build', 'title' => t('Building')];
        $menu_tree[] = ['href' => 'admin/settings', 'title' => t('Configuration')];
        $menu_tree[] = ['href' => 'admin/user', 'title' => t('Users')];
        $menu_tree[] = ['href' => 'admin/reports', 'title' => t('Reports')];
        if (module_exists('help')) {
            $menu_tree[] = ['href' => 'admin/help', 'title' => t('Help')];
        }
    } else {
    ...
    if ($menu_tree) {
      ...
      $class = '';
            ...
            $item['data'] = l($icon.$item['title'], $item['href'], ['html' => true]);
            if (!empty($id)) {
                $item['id'] = $id;
            }
            if (!empty($class)) {
...
function rootcandy_preprocess_page(&$vars)
{
  ...
  $admin_theme = variable_get('admin_theme', 'garland');
    if (arg(0) == 'admin' and arg(1) == 'settings' and arg(2) == 'admin' and ($admin_theme == 'rootcandy' or $admin_theme == 'rootcandy_fixed' or $admin_theme == 'rootcandy_dark')) {
        $message = t('Thank you for using RootCandy.<br />Did you know, that Root Candy has advanced settings (Theme-specific settings fieldset)? You can change these settings at <a href="@configure-page">theme configuration page</a>.', ['@configure-page' => url('admin/build/themes/settings/'.$admin_theme)]);
    ...
    global $theme_info;
        if (!$theme_info->status) {
            $warning_message = t('RootCandy theme has not been enabled. You won\'t be able to see theme updates on update page! Please enable the theme on <a href="@themes-page">themes page</a>.', ['@themes-page' => url('admin/build/themes')]);
  ...
  if (arg(0) == 'admin' and arg(2)) {
      ...
      $menu_links = _rootcandy_links($menu, ['id' => 'rootcandy-menu']);
...
function rootcandy_settings_init($theme)
{
  ...
  $defaults = (is_array($themes[$theme]->info['settings'])) ? $themes[$theme]->info['settings'] : [];
...
function _rootcandy_links($links, $attributes = ['class' => 'links'])
{
    $output = '';
...
function rootcandy_system_settings_form($form)
{
    $themes = list_themes();
    $enabled_theme = arg(4);
    if ($form['#id'] == 'system-theme-settings' and ($enabled_theme == 'rootcandy' || (isset($themes[$enabled_theme]) && in_array($themes[$enabled_theme]->base_theme, ['rootcandy', 'rootcandy_dark', 'rootcandy_fixed'])))) {
      ...
      if (intval($rid)) {
        ...
        $table_rows[$weight] = ['data' => $this_row, 'class' => 'draggable'];
      }
        }
        ksort($table_rows);
      ...
      'Role', 'Navigation menu', 'Order',
        ...
        $form['theme_specific']['navigation']['role-weights']['content']['#value'] = theme('table', $header, $table_rows, ['id' => 'rootcandy-settings-table']);
...
function rootcandy_theme()
{
    ...
    'admin_navigation' => [
      'arguments' => ['items' => null, 'class' => null],
    ],
  ];
}
